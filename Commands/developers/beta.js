...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('beta')
    .setDescription(
      'Add or Remove beta test program for a user. (Devs Limited)'
    )
    .addSubcommand(sub =>
      sub
        .setName('add')
  ...
  async execute(interaction, client) {
    if (interaction.options.getSubcommand() == 'add') {
      ...
      const SuccessEmbed = new EmbedBuilder()
        .setTitle('__Added__')
        .setDescription(
          `I'm are pleased to inform you that a new user has been successfully added to the beta test program. This program is specifically designed to evaluate and refine the performance, usability, and functionality of our software or service before its official release to the wider public.`
        )
        .addFields({
...
**\`»\` Tag**: \`${user?.username}\``,
      ...
      const SuccessEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `I acknowledge and respect your decision to request removal from our esteemed beta testing program. We deeply appreciate your participation thus far and understand that your circumstances or preferences may have changed, leading to your desire to discontinue your involvement.`
        )
        .addFields({
          name: '__User Information__',
          value: `**\`»\` ID**: \`${user.id}\`
...
**\`»\` Tag**: \`${user?.username}\``,
      ...
      if (!existingUser) {
        const exisiting = new EmbedBuilder()
          .setTitle('__Error__')
          .setDescription(
            `We regret to inform you that, as per our extensive database analysis, it has been determined that your esteemed persona is not currently present within our records. In order to proceed with the removal process, it is imperative that we first add your information to our database.`
          )
          .setColor(`#2F3136`)
          .setFooter({ text: 'Reliable | Your trusted assistant' });
        interaction.reply({ embeds: [exisiting], ephemeral: true });
